<template>
  <div :class="$style.container">
    <Cell :class="$style.header" v-if="chat">
      ...
      <Button
        class="size-lg padding-lg"
        color="text-danger"
        @click.native="removeChat"
      >
        <i class="fa fa-trash" />
        <span :class="$style.hiddenOnMobile"> Delete </span>
      </Button>
      <Button
        class="size-lg padding-lg"
        color="transparent"
        @click.native="closeChat"
      >
        <i class="fa fa-times" />
        <span :class="$style.hiddenOnMobile"> Close </span>
      </Button>
    </Cell>
    <div :class="$style.conversation" class="padding-top-lg" ref="conversation">
      <div
        ...
        :key="'m' + index"
        :class="[$style.messageItem, message.from, calcTextDir(message.body)]"
      >
        <div class="inside padding-top-md padding-x-md margin-y-xs text-md">
          {{ message.body }}
          <div class="info padding-bottom-md padding-top-sm">
            {{ message.date | datetime }}
          </div>
        </div>
      </div>
      <div :class="$style.isTyping" class="padding-sm">
        <span v-if="itIsTyping"> {{ chat.user.name }} is typing... </span>
      </div>
    </div>
    <MessageForm
...
export default {
  ...
  mounted() {
    this.$chatService.$on("isTypingFlag", user => {
      if (
        this.chat &&
        this.chat.user.username === user.username &&
        this.chat.user.type === user.type
      ) {
